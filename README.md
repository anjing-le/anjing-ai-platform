...
- 网关闭环：新增路由、控制台配置路由策略/权重/canary、发布路由、已发布 route 代理限流、多 upstream 候选、`ordered` / `round_robin` / `weighted` 策略、route 级策略/权重/canary 灰度持久化、请求级策略覆盖、route 级熔断冷却、模型路由创建与发布、Skill 绑定创建与发布、Skill 输入校验、LLM 调用测试、请求日志与用量联动
...
| `gateway` | `gateway-api` | `/api/gateway/routes`, `/api/gateway/routes/update`, `/api/gateway/routes/health-check`, `/api/gateway/routes/preflight`, `/api/gateway/proxy`, `/api/gateway/model-routes`, `/api/gateway/llm/invoke`, `/api/gateway/llm/stream`, `/api/gateway/request-logs`, `/api/gateway/request-logs/export`, `/api/gateway/request-logs/retention/purge` |
