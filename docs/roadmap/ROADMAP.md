...
- [ ] 增加页面级 loading、error retry 和后台数据刷新策略的可视化细节。
- [ ] 为关键操作补充二次确认和更明确的失败恢复路径。
- [ ] 增加更完整的表格筛选、排序、分页和导出能力。
- [ ] 增加控制台截图和模块截图，方便 public repo 快速理解项目形态。
- [ ] 把帮助文档从内嵌内容升级为更完整的接入指南、API 示例和部署说明。
- [ ] 完善移动端和窄屏后台体验，但仍以桌面管理后台为主要使用场景。
...
- [ ] 为计费增加幂等用量事件、预算阈值和账单聚合。
- [ ] 为审计和请求日志增加查询条件、保留策略和导出能力。
