...
- `POST /api/gateway/model-routes/update`
...
控制台进入后台后优先调用 `GET /api/ops/platform-snapshot`。这个接口一次性返回首页和模块入口需要的核心数据：运营指标、待办、健康、审计、用户、应用、角色、API Key、凭据、网关路由、模型路由、Skill、Skill schema registry、请求日志、套餐、账单汇总、用量和预算告警。
