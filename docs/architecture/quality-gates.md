...
- 帮助文档模块的用户可见 tab 必须使用 `Guides`、`API Examples`、`Deployment`、`Troubleshooting`，并提供可选择的文档详情、命令片段和排障路径。
...
- Guides 必须提供可复制的最小调用示例，让使用用户能从后台直接进入接入链路。
- 文档中心复制动作必须提供成功/失败反馈，失败提示必须作为状态区域暴露，并给代码片段保留可访问标签。
- Guides 在暂无应用时也必须展示占位调用示例，避免 Mock / 首次接入状态下断链。
- Guides 必须展示带可读摘要的最小接入清单和当前状态，让使用用户知道应用、Key、路由和调用验证的顺序。
- Guides 接入应用动作必须复用共享角色边界，只允许管理员、使用用户和开发人员执行，并暴露带权限原因的可访问标签。
