...
- 帮助文档模块的用户可见 tab 必须使用 `快速接入` 和 `常见问题`，避免 `Quickstart` / `FAQ` 这类混杂标签进入后台界面。
- 后端快照合成的模块页标题、指标说明和表头必须使用同一套中文后台语境，不能把 `Health`、`Applications`、`Go API live` 这类开发说明直接展示出来。
- 面板 eyebrow、空状态、表单标签和运行命令区等微文案必须保持中文后台语境，技术名仅在模块名、协议名、命令或 API 标识中保留。
...
- 后台通用 `Panel` 区域必须用 eyebrow 和标题暴露可访问区域标签，保证模块内容边界清晰。
- 后台详情空态必须复用 `EmptyPanel` 状态组件，暴露标题和说明摘要，避免首次接入或无数据时只剩视觉提示。
- 关键指标卡片必须暴露带状态等级的可读摘要，保证总览信息离开视觉布局后仍然完整。
- 后台首页的主运行命令必须可复制，保证本地联调入口可以从页面直接拿到。
- 后台首页的单服务运行命令必须可复制，保证后续按服务拆分联调时入口明确。
- 后台首页的运行命令复制动作必须提供成功/失败反馈，失败提示必须作为状态区域暴露，剪贴板不可用时提示手动复制。
- 所有复制按钮必须暴露明确可访问标签，并隐藏装饰性复制图标。
- 后端服务规划卡片和服务归属行必须暴露可读摘要，服务归属表头必须使用中文后台文案，保证拆分计划离开视觉表格后仍然清楚。
- 侧边栏必须暴露并渲染当前导航项，保证后台用户知道自己所在模块。
- 侧边栏导航项必须暴露明确目的地标签，并隐藏装饰性图标。
- 侧边栏品牌入口必须明确回到后台首页，避免 hash 路由下只跳回页面顶部。
- 顶部工具区必须保留手动刷新平台数据的动作，暴露刷新中状态，并隐藏装饰性刷新图标。
- 顶部状态徽标必须展示最近同步时间，未完成首次同步时展示等待状态。
- 顶部状态徽标必须区分聚合快照、分组接口和本地演示数据，避免只显示模糊在线状态。
- 顶部状态徽标必须作为 live region 暴露中文数据来源和同步时间，保证刷新反馈可访问。
- 顶部角色区必须展示中文角色视角和当前角色用途，避免用户切换视角后只能从按钮推断权限范围。
- 顶部角色切换必须暴露当前角色状态、角色用途和是否当前选中，并渲染当前状态，保证权限视角明确。
- 角色可见矩阵每行必须暴露当前角色可见性和可见角色摘要。
- 模块页主操作按钮必须暴露模块名、动作名和权限原因，并隐藏装饰性箭头。
- 装饰性箭头图标必须从辅助技术中隐藏，避免重复播报按钮或链接动作。
- 详情面板主动作必须暴露操作对象、进行中/完成状态和权限原因，保证路由、模型、Skill、计费、用户、密钥、凭据等后台动作语义稳定。
- 权限提示必须作为 `note` 暴露统一说明，保证禁用动作的原因在各模块里都能被识别。
- 操作通知必须在短时间后自动清理，避免后台页面堆积过期状态。
- 操作通知必须作为 polite status region 暴露，保证后台操作反馈可感知。
- 操作弹窗必须关联标题/说明，并把提交错误作为 alert 暴露，保证主流程表单可访问。
- 模块页 tab 必须使用 `tablist` / `tab` / `tabpanel` 语义，暴露当前选中状态，支持键盘切换，并提供可见焦点态。
- 模块工作流必须暴露每一步的完成状态、当前步骤和说明摘要，并隐藏装饰性步骤编号。
- 操作弹窗必须支持 `Escape` 关闭，并在提交中禁止关闭，避免主流程误中断。
- 操作弹窗打开后必须自动聚焦第一个字段，保证主流程表单可以直接键盘录入。
- 操作弹窗提交中必须暴露 `aria-busy` 和 live submit 状态，保证主流程状态可感知。
- 模块内联表单必须自动聚焦第一个可编辑控件，在提交中或只读角色下禁用可编辑控件、暴露 `aria-busy`，并把提交错误作为 alert 暴露。
- 模型路由和 Skill 创建表单必须使用中文后台标签，字段值可以保留协议、模型名和路由路径等技术标识。
- API Key 和凭据列表/详情字段必须使用中文后台标签，空到期时间必须展示中文说明。
- 模块内联表单提交按钮必须暴露当前操作对象和权限原因，保证创建模型路由、创建 Skill 和 LLM 调用动作语义清晰。
- 应用接入、预算告警和 LLM 调用测试等关键面板必须使用中文后台文案，字段值可以保留 provider、model、token 等技术标识。
- 状态枚举必须保留后端原始值，但状态徽标、筛选项和选中详情必须通过前端展示层显示中文状态。
- 模块表格搜索必须覆盖中文状态和下一步语义，用户输入 `预警`、`待处理`、`优先处理` 等词时可以命中对应原始枚举行。
- LLM 调用测试结果必须作为状态区域暴露 provider、model、兜底模型和 token 用量摘要。
- 模块页存在多个内联表单时，辅助表单必须显式 opt in 后才能抢占初始焦点，避免覆盖主操作表单。
- 后台首页模块入口必须展示当前角色可进入/受限模块数量，并在搜索生效时提供 `清空搜索`。
- 后台首页主接入入口、模块搜索清空和模块表格筛选清空动作必须暴露明确标签，保证快捷操作可被稳定识别。
- 首屏架构模块卡片必须暴露明确目的地标签，并隐藏装饰性图标，保证从 landing 到 console 的入口语义一致。
- 后台首页可进入模块必须暴露明确目的地标签，保证模块入口语义稳定。
- 后台首页模块卡片图标必须作为装饰图标隐藏，避免重复播报模块名和入口信息。
- 后台首页和模块页搜索输入必须有明确可访问标签，模块表格搜索占位必须使用中文后台文案，不能只依赖 placeholder。
- 搜索输入内的装饰性图标必须从辅助技术中隐藏，避免重复播报。
- 后台首页受限模块必须说明哪些角色可进入，避免用户只能看到模糊的不可见状态。
- 后台首页待办区必须聚焦未完成事项，展示待处理/已处理数量，并在全部完成时给出空状态。
- 后台首页待办区只展示前几条时必须说明仍有多少隐藏事项，并提供进入运营总览查看全部的入口。
- 后台首页模块搜索无结果和待办清空状态必须作为可读状态区域暴露，避免空页面只依赖视觉提示。
- 后台首页模块搜索无结果建议必须使用中文业务关键词；后端服务规划标题必须使用中文业务服务名，服务 label 和命令可以保留英文技术标识。
- 后台首页待办查看链接必须说明要进入哪个待办所属模块，避免多个 `查看` 链接语义重复。
- 后台首页待办处理按钮必须暴露带权限原因的可访问标签。
- 模块表格必须展示过滤后的行数和总行数，保证搜索/状态筛选反馈明确。
- 后台首页模块统计和表格结果统计必须使用中文单位，避免 `modules` / `rows` 这类界面单位混杂。
- 装饰性状态图标必须从辅助技术中隐藏，避免和文字状态重复播报。
- 状态徽标必须暴露统一的可读状态标签，保证待办、表格、详情和快速接入中的状态语义一致。
- 模块表格必须保留可访问表格标签、可选择行标签和当前选中行状态，保证后台列表组件可复用。
- 模块表格可访问标签必须使用中文连接格式，避免后台读屏标签出现英文式分隔符。
- 选中详情必须使用中文下一步提示，并暴露包含当前状态的可读摘要。
- 详情摘要顶部标签必须使用中文后台文案，不能暴露 `Selected ...` 或未本地化的到期说明。
- 模块表格在搜索或状态筛选生效时必须提供 `清空筛选`，避免用户卡在局部视图里。
- 模块表格状态筛选必须有明确可访问标签。
- 模块表格必须提供上下文化空状态，区分筛选无结果和模块暂无记录，并作为状态区域暴露。
- 快速接入必须提供可复制的最小调用示例，让使用用户能从后台直接进入接入链路。
- 快速接入复制动作必须提供成功/失败反馈，失败提示必须作为状态区域暴露，并给代码片段保留可访问标签。
- 快速接入在暂无应用时也必须展示占位调用示例，避免 Mock / 首次接入状态下断链。
- 快速接入必须展示带可读摘要的最小接入清单和当前状态，让使用用户知道应用、Key、路由和调用验证的顺序。
- 快速接入接入应用动作必须复用共享角色边界，只允许管理员、使用用户和开发人员执行，并暴露带权限原因的可访问标签。
- 快速接入调用示例必须指向默认 `platform-all` API 地址 `http://localhost:18080`。
