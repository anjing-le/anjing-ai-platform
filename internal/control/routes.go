...
func rotateApplicationKeyHandler(applications ApplicationRepository) http.HandlerFunc {
	type rotateApplicationKeyRequest struct {
		ID string `json:"id"`
	}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req rotateApplicationKeyRequest
		if err := httpjson.Decode(r, &req); err != nil {
			httpjson.BadRequest(w, err.Error())
			return
		}
		...
		item, ok, err := applications.RotateApplicationKey(r.Context(), req.ID)
		if err != nil {
			httpjson.Fail(w, http.StatusInternalServerError, "internal_error", err.Error())
			return
		}
