...
| `scripts/check-local-dev-scripts.mjs` | `pnpm verify:local-dev` | 检查本地数据库准备、Go API 启动和控制台 API 连接配置是否保持一键可用。 |
| `scripts/smoke-platform-api.mjs` | `pnpm verify:smoke-api` | 启动 `cmd/platform-all` 并检查 `/healthz` 和聚合快照接口可用。 |
